...
},{"core-js/fn/regexp/escape":2,"core-js/shim":325,"regenerator-runtime/runtime":327}],2:[function(require,module,exports){
...
forms.forEach(function (form) {
  var elements = form.querySelectorAll(fieldSelector);
...
window.addEventListener('paymentSystemInfo', function (event) {
  var paymentSystemInfo = event.detail;
...
},{"./module/mask":333,"./module/validation":335,"babel-polyfill":1}],332:[function(require,module,exports){
"use strict";
...
exports.customEventFunction = customEventFunction;
...
function customEventFunction(event, params) {
...
function prepareMaskArray(array) {
  var result = [];
  var patterns = Object.keys(maskPattern);

  array.forEach(function (item) {
    if (typeof item === 'string') {
      [].concat(_toConsumableArray(item)).forEach(function (symbol) {
...
var presetMask = exports.presetMask = {
  ...
  cardnumber: {
    mask: function mask(rawValue) {
      ...
      if (paymentSystemInfo) {
        var cardLengthList = paymentSystemInfo.cardLength.filter(function (key) {
          return key >= inputCardLength;
        });

        if (cardLengthList && cardLengthList.length) {
          maskNumLength = Math.min.apply(Math, _toConsumableArray(cardLengthList));
        }
      }
  ...
  cvv2: {
    mask: function mask() {
      ...
      var maskNumLength = valueRangeList[valueRangeList.length - 1];

      var stringMask = '#'.repeat(maskNumLength);
  ...
  email: {
    mask: function mask(rawValue, config) {
  ...
  phone: {
    mask: function mask(rawValue) {
      var arrayMask = ['+7 (', /[1-9]/, '##) ###-##-##'];
      var startSymbolPosition = rawValue.search(/[+1-9]/);
      var startSymbol = rawValue[Number(startSymbolPosition)];
...
function setMask(inputElement, maskName) {
  var presetMaskKeys = Object.keys(presetMask);

  if (presetMaskKeys.includes(maskName)) {
    var _presetMask$ = presetMask['' + maskName],
        mask = _presetMask$.mask,
        pipe = _presetMask$.pipe,
        guide = _presetMask$.guide,
        placeholderChar = _presetMask$.placeholderChar;
...
},{"./paymentSystem":334,"text-mask-addons/dist/createAutoCorrectedDatePipe":328,"text-mask-addons/dist/emailMask":329,"vanilla-text-mask":330}],334:[function(require,module,exports){
'use strict';
...
var PaymentSystemDefault = exports.PaymentSystemDefault = {
  name: '',
  range: [],
...
var paymentSystemList = exports.paymentSystemList = [{
  name: 'amex',
  range: [{
    start: 340000,
    end: 349999
...
function getPaymentSystemInfoByPan(pan) {
  var binStart = void 0;
  var binEnd = void 0;
  var paymentSystemInfo = PaymentSystemDefault;

  if (pan.length >= 6) {
    var strBin = pan.substr(0, 6);

    binStart = Number(strBin);
    binEnd = Number(strBin);
  } else {
    var strBinStart = (pan + '00000').substr(0, 6);
    var strBinEnd = (pan + '99999').substr(0, 6);

    binStart = Number(strBinStart);
    binEnd = Number(strBinEnd);
  }

  paymentSystemList.forEach(function (paymentSystem) {
    var binRangeList = paymentSystem.range;

    if (binRangeList.some(function (range) {
      return range.start <= binStart && binEnd <= range.end;
    })) {
...
},{"./customEvent":332}],335:[function(require,module,exports){
'use strict';
...
var validationPreset = exports.validationPreset = {
  ...
  cardnumber: function cardnumber(rawValue, isRequired) {
    var value = rawValue.replace(/\D/g, '');
    var valueRangeList = [14, 15, 16, 17, 18, 19];
  ...
  city: function city(rawValue, isRequired) {
    var value = rawValue.trim();
  ...
  expdate: function expdate(rawValue, isRequired) {
    var value = rawValue.trim();
    var currentDate = (0, _dayjs2.default)().set('date', 1).set('hour', 0).set('minute', 0).set('second', 0).set('millisecond', 0);

    var minDate = (0, _dayjs2.default)(currentDate).add(-1, 'millisecond');

    var maxDate = (0, _dayjs2.default)(currentDate).add(5, 'year').add(1, 'month');

    if (value !== '') {
      var rule = /[^0-9]+/;

      var expDateList = value.split(rule);
      var month = Number(expDateList[0]);
      var year = Number(expDateList[1]);
  ...
  phone: function phone(rawValue, isRequired) {
    var value = rawValue.replace(/\D/g, '');
...
function isValid(value, valueType, isRequired) {
  var validationPresetKeys = Object.keys(validationPreset);
